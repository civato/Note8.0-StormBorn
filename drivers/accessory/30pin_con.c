...
struct acc_con_info {
	...
	int cable_sub_type;
	int cable_pwr_type;
#if defined(CONFIG_SAMSUNG_MHL_9290) || defined(CONFIG_MHL_SII9234)
	bool mhl_pwr_state;
#endif
...
void acc_accessory_uevent(struct acc_con_info *acc, int acc_adc)
{
	...
	if (acc_adc != false) {
		if ((1100 < acc_adc) && (1400 > acc_adc)) {
			...
			acc->pdata->acc_power(3, true);
			acc_notify(MHL_1X_ATTACH);
#endif
		} else {
		...
		if (acc->current_accessory == ACCESSORY_OTG)
			...
			env_ptr = "ACCESSORY=MHL";
			acc_notify(MHL_1X_DETACH);
			acc->pdata->acc_power(3, false);
		}
#endif
		else
			env_ptr = "ACCESSORY=unknown";
...
#if defined(CONFIG_SAMSUNG_MHL_9290)
				acc_notify(MHL_3X_ATTACH);
#elif defined(CONFIG_MHL_SII9234)
				sii9234_tpi_init();
#endif
