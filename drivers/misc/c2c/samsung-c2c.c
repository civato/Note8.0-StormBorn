			...
			const char *buf, size_t count)
{
	...
	case 1:
		...
		c2c_set_sharedmem(c2c_con.shd_mem_size, c2c_con.shdmem_addr);
...
int c2c_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	unsigned long pfn = __phys_to_pfn(c2c_con.shd_mem_base + offset);

	if ((offset + size) > c2c_con.shd_mem_size) {
		pr_err("[C2C] ERR! (offset + size) > %lu MB\n",
			c2c_con.shd_mem_size >> 20);
		return -EINVAL;
	}
...
static int c2c_set_sharedmem(enum c2c_shrdmem_size size, dma_addr_t addr)
{
	dma_addr_t shm_base = addr;
	size_t alloc = 1 << (22 + size);

	if (addr == 0) {
		pr_err("[C2C] %s: call cma_alloc() ...\n", __func__);
		shm_base = cma_alloc(c2c_con.c2c_dev, "c2c_shdmem", alloc, 0);
		if (IS_ERR_VALUE(shm_base)) {
			pr_err("[C2C] %s: cma_alloc fail\n", __func__);
			cma_free(shm_base);
			return -ENOMEM;
		}
		c2c_con.shdmem_addr = shm_base;
	}

	c2c_con.shd_mem_base = shm_base;
	c2c_con.shd_mem_size = alloc;
	pr_err("[C2C] %s: base 0x%08X, size %d MB\n",
		__func__, (int)shm_base, (alloc >> 20));

	/* Set DRAM Base Addr & Size */
	c2c_set_shdmem_size(size);
	c2c_set_base_addr((shm_base >> 22));
...
static irqreturn_t c2c_sscm1_irq(int irq, void *data)
{
	...
	pr_debug("[C2C] %s: interrupt 0x%08X\n", __func__, raw_irq);
...
static void set_c2c_device(struct platform_device *pdev)
{
	...
	clk_enable(clk_get(&pdev->dev, "c2c"));
...
bool c2c_is_suspended(void)
{
	return c2c_con.suspended;
}
EXPORT_SYMBOL(c2c_is_suspended);
#else
