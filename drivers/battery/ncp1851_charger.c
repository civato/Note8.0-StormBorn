...
#define CTRL2_AICL_ENABLED                    BIT(0)
#define CTRL2_IINLIM_ENABLED                  BIT(1)
#define CTRL2_TRANS_ENABLED                   BIT(4)
...
static int ncp1851_get_charging_status(struct i2c_client *client)
{
...
#if defined(CONFIG_MACH_KONA)
	ncp1851_i2c_read(client, NCP1851_VIN_SNS, &vin_sns);
	dev_info(&client->dev,
		"%s : charger VIN_SNS Register(0x%02x)\n",
		__func__, vin_sns);

	ncp1851_i2c_read(client, NCP1851_VBAT_SET, &vbat_reg);
	dev_info(&client->dev,
		"%s : charger VBAT_SET Register(0x%02x)\n",
		__func__, vbat_reg);

	/* Check VIN voltage detection */
	if ((vin_sns & 0x08) == 0x08) {
		if ((vbat_reg & 0x0c) == 0x0c) {
			dev_info(&client->dev,
				"%s : Set float voltage.\n", __func__);
			data = 0x28;
			ncp1851_i2c_write(client, NCP1851_VBAT_SET, &data);
			...
			/* Rewrite register */
			ncp1851_charger_function_control(client);
	...
	/* Charge mode : DPP mode */
	if ((stat_reg & 0x70) == 0x70)
		goto charging_status_end;
...
static void ncp1851_charger_function_control(
				struct i2c_client *client)
{
	...
	if (charger->cable_type ==
		...
		/* Check charging mode */
		if (charger->charging_mode == SEC_BATTERY_CHARGING_2ND) {
			/* Set CHG_EN = 0 */
			cur_reg = 0x00;
			ncp1851_i2c_write(client, NCP1851_CTRL1, &cur_reg);

			/* Change IEOC = 250mA to 150mA */
			data |= (ncp1851_get_termination_current_limit_data(
				charger->pdata->charging_current[
				charger->cable_type].full_check_current_2nd) << 4);
		...
		/* set float voltage. default 4.2v */
		...
		** if cable type is USB, Trans pin set enable
		*/
		if (charger->cable_type == POWER_SUPPLY_TYPE_USB) {
			ctrl2_reg &= ~CTRL2_AICL_ENABLED;
		}

		/* Trans pin always set enable */
		ctrl2_reg |= CTRL2_TRANS_ENABLED;

		/* set input current limit disable, if input current
		** over 1500mA. (for NCP1852)
		*/
		if (charger->pdata->charging_current
			[charger->cable_type].input_current_limit > 1500)
			ctrl2_reg &= ~CTRL2_IINLIM_ENABLED;

		ncp1851_i2c_write(client, NCP1851_CTRL2, &ctrl2_reg);
	}
}

#if defined(CONFIG_MACH_KONA)
static void ncp1851_siop_control(
				struct i2c_client *client)
{
	struct sec_charger_info *charger = i2c_get_clientdata(client);
	u8 data = 0;
...
static void ncp1851_charger_otg_control(
				struct i2c_client *client)
{
	struct sec_charger_info *charger = i2c_get_clientdata(client);
}
			      ...
			      union power_supply_propval *val)
{
	...
	/* KONA: Workaround for supporting ncp1852 charger ic. */
#if defined(CONFIG_MACH_KONA_EUR_OPEN) ||\
	defined(CONFIG_MACH_KONA_EUR_WIFI) || defined(CONFIG_MACH_KONA_KOR_WIFI)
	if (system_rev == 1 || system_rev == 2) {
#else
	if (system_rev == 1) {
#endif
			ncp1851_i2c_read(client, NCP1851_CH2_INT, &chg_reg);

		if (chg_reg & 0x02) {
			cur_reg = 0x41;
			dev_dbg(&client->dev,
			"%s : Charger timer expired. Restart charging.\n",
			__func__);
			ncp1851_i2c_write(client, NCP1851_CTRL1, &cur_reg);
		}
	}
