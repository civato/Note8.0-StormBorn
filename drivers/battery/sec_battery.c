				...
				bool enable)
{
	...
	if (enable) {
		...
		/* Set charging mode before set current value */
...
#if defined(CONFIG_MACH_KONA)
static void sec_bat_check_charge_current(
				struct sec_battery_info *battery)
{
	union power_supply_propval val;
	static bool run_siop = false;
	int chg_curr;

	if ((battery->cable_type != POWER_SUPPLY_TYPE_BATTERY) && 
		(battery->siop_state == true)) {
	
		switch (battery->siop_lv) {
		case SIOP_LV1:
			battery->siop_charge_current =
				battery->pdata->chg_curr_siop_lv1;
			break;
		case SIOP_LV2:
			battery->siop_charge_current =
				battery->pdata->chg_curr_siop_lv2;
			break;
		case SIOP_LV3:
		default:
			battery->siop_charge_current =
				battery->pdata->chg_curr_siop_lv3;
			break;
		}
		...
		chg_curr = val.intval;
		val.intval = battery->siop_charge_current;

		pr_info("%s: siop state, level(%d), cc(%d)\n",
				__func__, battery->siop_lv, chg_curr);

		if (chg_curr != battery->siop_charge_current) {
			/* Update current value :SIOP */
			psy_do_property("sec-charger", set,
				POWER_SUPPLY_PROP_CURRENT_NOW, val);
		}
	} else if ((battery->cable_type != POWER_SUPPLY_TYPE_BATTERY) && 
		(!battery->siop_state) && run_siop) {
			sec_bat_set_charge(battery, true);
			run_siop = false;
	}
}
...
#if defined(CONFIG_MACH_KONA)
	/* 6. Check current update */
	sec_bat_check_charge_current(battery);
#endif
