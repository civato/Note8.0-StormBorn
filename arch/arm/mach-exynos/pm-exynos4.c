...
#if defined(CONFIG_MACH_M0_CTC)
{
	...
	tmp = __raw_readl(S5P_EINT_PEND(0));
	printk(KERN_DEBUG "[kona] check pend0 %lu\n", tmp);
	tmp = __raw_readl(S5P_EINT_PEND(1));
	printk(KERN_DEBUG "[kona] check pend1 %lu\n", tmp);
	tmp = __raw_readl(S5P_EINT_PEND(2));
	printk(KERN_DEBUG "[kona] check pend2 %lu\n", tmp);
	tmp = __raw_readl(S5P_EINT_PEND(3));
	printk(KERN_DEBUG "[kona] check pend3 %lu\n", tmp);
#endif
