...
static void set_wake_locked(int wake)
{
	...
	gpio_set_value(bt_wake_pin/*GPIO_BT_WAKE*/, wake);
}
...
static int bcm4334_bluetooth_probe(struct platform_device *pdev)
{
	...
	if (system_rev >= 3)
		bt_wake_pin = GPIO_BT_NEW_WAKE;
	else
		bt_wake_pin = GPIO_BT_WAKE;
#else
	bt_wake_pin = GPIO_BT_WAKE;
#endif

	rc = gpio_request(bt_wake_pin/*GPIO_BT_WAKE*/, "bcm4334_btwake_gpio");
	...
	if (unlikely(rc)) {
		...
		gpio_free(bt_wake_pin/*GPIO_BT_WAKE*/);
		gpio_free(GPIO_BT_EN);
		return rc;
	}
	gpio_direction_input(GPIO_BT_HOST_WAKE);
	gpio_direction_output(bt_wake_pin/*GPIO_BT_WAKE*/, 0);
