...
#if defined(CONFIG_LEGACY_HOTPLUG_POLICY_SCREEN_OFF_ONLY)
static void pm_hotplug_early_suspend(struct early_suspend *h)
{
	mutex_lock(&hotplug_lock);
	queue_delayed_work_on(0, hotplug_wq, &hotplug_work, 5 * HZ);
	mutex_unlock(&hotplug_lock);
}

static void pm_hotplug_late_resume(struct early_suspend *h)
{
	mutex_lock(&hotplug_lock);
	cancel_delayed_work_sync(&hotplug_work);

	if (cpu_online(1) == 0) {
		pr_info("cpu1 turning on!(%s)\n",__func__);
		cpu_up(1);
	}
...
static struct early_suspend pm_hotplug_early_suspend_desc = {
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB,
	.suspend = pm_hotplug_early_suspend,
	.resume = pm_hotplug_late_resume,
};
#endif
...
#if defined(CONFIG_LEGACY_HOTPLUG_POLICY_ALWAYS_ON)
	queue_delayed_work_on(0, hotplug_wq, &hotplug_work, 60 * HZ);
#elif defined(CONFIG_LEGACY_HOTPLUG_POLICY_SCREEN_OFF_ONLY)
	register_early_suspend(&pm_hotplug_early_suspend_desc);
#endif
