...
void __init midas_wacom_init(void)
{
	...
	if (ret) {
		printk(KERN_ERR "[E-PEN] failed to request PEN_RESET.(%d)\n",
			ret);
		return ;
	}
	...
	if (system_rev < WACOM_FWE1_HWID) {
		...
		if (ret) {
			printk(KERN_ERR "[E-PEN] failed to request PEN_SLP.(%d)\n",
				ret);
			return ;
		}
	...
	if (ret) {
		printk(KERN_ERR "[E-PEN] failed to request PEN_PDCT.(%d)\n",
			ret);
		return ;
	}
	...
	if (ret) {
		printk(KERN_ERR "[E-PEN] failed to request PEN_IRQ.(%d)\n",
			ret);
		return ;
	}
	s3c_gpio_setpull(gpio, S3C_GPIO_PULL_NONE);
	...
	if (ret) {
		printk(KERN_ERR "[E-PEN] failed to request PEN_LDO_EN.(%d)\n",
			ret);
		return ;
	}
	s3c_gpio_cfgpin(gpio, S3C_GPIO_OUTPUT);
